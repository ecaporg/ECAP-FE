...
export const getStatusForDashboard = (item: StatsItem): LPStatusForDashboard => {
...
export const getLPNameForDashboard = (learningPeriods: ITrackLearningPeriod[]) => {
  return learningPeriods
    .map((lp, index, arr) => `<b>${`${lp.track.name}:`}</b> ${getShortLearningPeriodName(lp.name)}`)
    .join(', ');
};

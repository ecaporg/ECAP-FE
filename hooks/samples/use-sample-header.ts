...
export const sampleHeaderSchema = z.object({
  student_name: z
    .string()
    .min(1, { message: validationMessages.required("Student Name") }),
  course_title: z
    .string()
    .min(1, { message: validationMessages.required("Course Title") }),
  assignment_title: z
    .string()
    .min(1, { message: validationMessages.required("Assignment Title") }),
...
export function useSampleHeader({ sample }: { sample: ISample }) {
  ...
  const onBlur = async (
    ...
    if (e.target.value !== e.target.defaultValue) {
      ...
      if (isAnyAdmin(user)) {
        await flagCompletedSampleAction(sample, {
          message: `changed ${label} from ${
            e.target.defaultValue || "N/A"
          } to ${e.target.value}`,
        } as any);
      }
